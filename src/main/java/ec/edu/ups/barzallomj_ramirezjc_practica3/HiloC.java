/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package ec.edu.ups.barzallomj_ramirezjc_practica3;

import javax.swing.ImageIcon;

/**
 *
 * @author casa
 */
public class HiloC extends Thread {
    private int tiempo;

    public HiloC(int tiempo) {
        this.tiempo = tiempo;
    }

    @Override
    public void run() {
        int numero = 1;
        if (ec.edu.ups.barzallomj_ramirezjc_practica3.vista.VistaPrincipal.jLabel13.getIcon() != null) {
            String rutaImagen = ec.edu.ups.barzallomj_ramirezjc_practica3.vista.VistaPrincipal.jLabel13.getIcon().toString();

            int inicio = rutaImagen.lastIndexOf("\\") + 1;
            int fin = rutaImagen.lastIndexOf(".");

            if (inicio > 0 && fin > inicio) {
                String numeroImagen = rutaImagen.substring(inicio, fin);

                try {
                    numero = Integer.parseInt(numeroImagen);
                } catch (NumberFormatException e) {
                    numero = 1;
                }
            }
        }
        while (true) {
            String ruta = "src\\main\\resources\\imagenes\\" + numero + ".jpg";
            ImageIcon imagen = new ImageIcon(ruta);
            ec.edu.ups.barzallomj_ramirezjc_practica3.vista.VistaPrincipal.jLabel13.setIcon(imagen);

            int siguienteNumero = (numero % 4) + 1; 
            String ruta1 = "src\\main\\resources\\imagenes\\" + siguienteNumero + ".jpg";
            ImageIcon imagen1 = new ImageIcon(ruta1);
            ec.edu.ups.barzallomj_ramirezjc_practica3.vista.VistaPrincipal.jLabel11.setIcon(imagen1);

            int siguienteNumero2 = ((numero + 1) % 4) + 1;
            String ruta2 = "src\\main\\resources\\imagenes\\" + siguienteNumero2 + ".jpg";
            ImageIcon imagen2 = new ImageIcon(ruta2);
            ec.edu.ups.barzallomj_ramirezjc_practica3.vista.VistaPrincipal.jLabel14.setIcon(imagen2);

            int siguienteNumero3 = ((numero + 2) % 4) + 1;
            String ruta3 = "src\\main\\resources\\imagenes\\" + siguienteNumero3 + ".jpg";
            ImageIcon imagen3 = new ImageIcon(ruta3);
            ec.edu.ups.barzallomj_ramirezjc_practica3.vista.VistaPrincipal.jLabel12.setIcon(imagen3);

            numero = siguienteNumero;
            try {
                Thread.sleep(this.tiempo);
            } catch (Exception e) {
                System.out.println(e);
            }
        }
    }
}
